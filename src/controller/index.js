...
exports.getReportById = async (req, res) => {
  ...
  // console.log(reportFounddById);
      const decrypt = await CryptoJS.AES.decrypt(reportFounddById.description, 'secret key 123')
    var originalText = decrypt.toString(CryptoJS.enc.Utf8);
    reportFounddById.description = originalText;
  res.send(reportFounddById);
};

exports.getReportByAdminId = async (req, res) => {
  

  // const reportFounddById = await Report.find({adminId:  req.body.adminId}).select("descrption")
  const reportFounddById = await Report.where("adminId").equals(req.body.adminId)
  
  if(!reportFounddById) { throw new Error("Nothing found under this admin ID")}
  // const decrypt = await CryptoJS.AES.decrypt(reportFounddById.description, 'secret key 123')
  // var originalText = decrypt.toString(CryptoJS.enc.Utf8);
  // reportFounddById.description = originalText;
  
  reportFounddById.map( report => {
      const decrypt = CryptoJS.AES.decrypt(report.description, 'secret key 123')
      var originalText = decrypt.toString(CryptoJS.enc.Utf8);
      report.description = originalText;
  })

  // const descr = reportFounddById[0].description
  console.log(reportFounddById)
